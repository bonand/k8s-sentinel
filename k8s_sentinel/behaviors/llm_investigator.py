...
"""Agentic investigation via @llm_behavior con tool loop."""
import os
...
@llm_behavior(
    name="sentinel.llm_investigator",
...
def llm_investigator(event, graph, ctx, llm_output: InvestigationResult):
    ...
    if rem.requires_approval:
        ...
        robj = graph.add_object("Remediation", {
            ...
            "anomaly_id": anomaly.id, "proposed_at": _now()},
            actor="sentinel.llm_investigator", caused_by=event.id)
        graph.add_relation(anomaly.id, robj.id, "HAS_REMEDIATION",
                           {"created_at": _now()},
                           actor="sentinel.llm_investigator", caused_by=event.id)

    if llm_output.alert_team and llm_output.alert_message:
        alert = graph.add_object("Alert", {
            "severity": llm_output.severity, "message": llm_output.alert_message,
            "anomaly_id": anomaly.id, "status": "pending", "created_at": _now()},
            actor="sentinel.llm_investigator", caused_by=event.id)
        graph.add_relation(anomaly.id, alert.id, "TRIGGERED_ALERT",
                           {"created_at": _now()},
                           actor="sentinel.llm_investigator", caused_by=event.id)
        graph.emit("alert.created", {"alert_id": alert.id,
                   "anomaly_id": anomaly.id, "severity": llm_output.severity})
